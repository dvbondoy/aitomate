...
def _proc_result(result: subprocess.CompletedProcess) -> Dict[str, object]:
...
def append_log(path: str, text: str) -> Dict[str, object]:
    try:
...
def run_command(command: str, timeout: int = 30) -> Dict[str, object]:
...
def ping_host(host: str, count: int = 4, timeout: int = 2) -> Dict[str, object]:
    if not host.strip():
    ...
    command = [ping_bin, "-c", str(count), "-W", str(timeout), host]
...
def ssh_command(
    ...
    ssh_cmd = [ssh_bin, "-p", str(port), "-o", "BatchMode=yes"]
